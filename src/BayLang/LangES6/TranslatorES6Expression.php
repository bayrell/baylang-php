...
class TranslatorES6Expression extends \Runtime\BaseObject
{
	var $translator;
	...
	function OpCall($op_code, $result)
	{
		...
		if ($op_code->item instanceof \BayLang\OpCodes\OpIdentifier && $op_code->item->value == "parent")
		{
			...
			{
				$is_html = true;
				$result->push($this->translator->parent_class_name . ".methods." . $this->translator->class_function->name . ".call");
			}
			else if ($this->translator->class_function && $this->translator->class_function->name != "constructor")
			{
				$result->push("super." . $this->translator->class_function->name);
			}
			else
			{
				$result->push("super");
	...
	static function getClassName(){ return "BayLang.LangES6.TranslatorES6Expression"; }
